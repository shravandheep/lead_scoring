...
ordinal_feat_standardize = ['TIMESINCELEADCREATION', 'TIMEDIFF', 'TIME_TO_OUTBOUND_CALL', 'TIMETONEXTCALL']
ordinal_feat_normalize =    ['Cumulative_NVMConnect__NextContactTime__c',
                             'Cumulative_Outbound_Calls__c',
                             'Cumulative_Owner_changes',
                             'Cumulative_Phone_changes',
                             'Cumulative_Detail_changes']
...
def Ordinal_FeatEngg(lead_history_df_merged_subsetcols):
    ...
    for f in ordinal_feat_standardize+ordinal_feat_normalize:
        ...
        lead_history_df_merged_subsetcols[f] = encoder.inverse_transform([lead_history_df_merged_subsetcols[f]])
...
def generate_time_based_feat(lead_history_df):
    ...
    lead_history_df['TIMEDIFF'] = lead_history_df['update_time'].diff().dt.total_seconds().fillna(0)

    #### NVM_Next_Contact_time
    NEXTCALL = lead_history_df['field_changed'] == 'NVMConnect__NextContactTime__c'
    lead_history_df.loc[NEXTCALL, 'new_value'] = pd.to_datetime(lead_history_df.loc[NEXTCALL, 'new_value'], format='%Y-%m-%dT%H:%M:%S.%f')
    lead_history_df.loc[NEXTCALL, 'old_value'] = pd.to_datetime(lead_history_df.loc[NEXTCALL, 'old_value'], format='%Y-%m-%dT%H:%M:%S.%f')
    lead_history_df.loc[NEXTCALL, 'TIMETONEXTCALL'] = (lead_history_df[NEXTCALL]['new_value'] - lead_history_df[NEXTCALL]['old_value'])

    ### TIME BETWEEN CALLS
    OUTBOUNDCALLS = lead_history_df['field_changed'] == 'Outbound_Calls__c'
    lead_history_df.loc[OUTBOUNDCALLS, 'TIME_NEXT_CALL'] = lead_history_df[OUTBOUNDCALLS]['update_time'].diff().dt.total_seconds()
...
def generate_sequences(X):
    ...
    for i in range(min_sequence_length, number_of_sequences):
        sequence = torch.tensor(features[min_sequence_length -1 : i], dtype=torch.float)
        sequence = torch.nn.functional.pad(sequence, pad=(0, 0, 0, max(0, max_length_events - sequence.size(0))))
        X_.append(sequence)
...
def generate_df(lead_history):
    ...
    lead_history_df_not_status = pd.concat([lead_history_df_not_status, pd.get_dummies(lead_history_df_not_status['field_changed'])], axis=1)
    lead_history_df = pd.concat([lead_history_df_not_status, lead_history_df[lead_history_df['field_changed']=='Status']], axis=0).sort_values(by=['update_time'])
    ...
    lead_history_df_merged_ = pd.concat([lead_history_df, lead_history_df[set(reverse_field_mapping.values())].cumsum(axis=0).add_prefix('Cumulative_')], axis=1)

    ### one hot encoding Status changes 
    STATUS = lead_history_df_merged_['field_changed'] == 'Status'
    lead_history_df_merged_.loc[STATUS, 'Status'] = lead_history_df_merged_.loc[STATUS, 'new_value']
    lead_history_df_merged_['Status'] = lead_history_df_merged_['Status'].fillna(method='ffill').fillna('New')

    lead_history_df_merged_ = pd.concat([lead_history_df_merged_, pd.get_dummies(lead_history_df_merged_['Status']).add_prefix('Status_')], axis=1)
