...
def split_and_create_dict(data):
    ...
    values = data.split('|')[:8]
    keys = ['overall', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
...
def calculate_call_window_difference(row):
    ...
    if expected_window == 0:
        ...
        diff = min(abs(expected_window - actual_window), (abs(abs(actual_window - expected_window) - 12)))
        return abs(diff)
...
df_leads_neu['call_window'] = df_leads_neu['Input Phone1 Call Window'].apply(split_and_create_dict)
...
df_leads_neu['Expected_Call_Window'] = df_leads_neu.apply(extract_value, axis=1)

df_leads_neu['Overall_Call_Window'] = df_leads_neu.apply(extract_value, axis=1)

df_leads_neu['Actual_Call_Window'] = (df_leads_neu['CreatedDate'].dt.hour/2).astype(int) + 1
df_leads_neu['Daywise_window_diff'] = df_leads_neu.apply(calculate_call_window_difference, axis=1)

weight_daywise = 0.7
weight_overall = 0.3

df_leads_neu['Call_window_score'] = df_leads_neu.apply(calculate_combined_score, axis=1)
